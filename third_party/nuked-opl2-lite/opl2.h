...
/* Modified for Restunts, 2026-09-21: declare the public shared-library API.
 * Structure layouts and calling conventions remain those of upstream 0.9 beta. */
#if defined(_WIN32) && defined(NUKED_OPL2_SHARED)
# ifdef NUKED_OPL2_BUILD
#  define OPL2_API __declspec(dllexport)
# else
#  define OPL2_API __declspec(dllimport)
# endif
#else
# define OPL2_API
#endif
...
OPL2_API void OPL2_Generate(opl2_chip *chip, int16_t *buf);
OPL2_API void OPL2_GenerateResampled(opl2_chip *chip, int16_t *buf);
OPL2_API void OPL2_Reset(opl2_chip *chip, uint32_t samplerate);
OPL2_API void OPL2_WriteReg(opl2_chip *chip, uint8_t reg, uint8_t v);
OPL2_API void OPL2_WriteRegBuffered(opl2_chip *chip, uint8_t reg, uint8_t v);
OPL2_API void OPL2_GenerateStream(opl2_chip *chip, int16_t *sndptr, uint32_t numsamples);
OPL2_API uint8_t OPL2_ReadStatus(opl2_chip *chip);
